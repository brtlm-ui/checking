...
try {
    ...
    $amStatus = calculateStatus($currentDateTime, $officialAMIn, $officialTime['grace_period_minutes']);

    // Create time record with AM clock in
    $stmt = $conn->prepare("
        INSERT INTO time_record (employee_id, clock_in_am, am_status, created_at) 
        VALUES (?, ?, ?, ?)
    ");
    $stmt->execute([$employeeId, $currentDateTime, $amStatus, $currentDateTime]);
    ...
    // Set success message
    $_SESSION['success_message'] = "Successfully clocked in at " . formatTime($currentDateTime) . " - Status: " . $amStatus;
